...
void *io_thread_a2dp_source_aptx(void *arg) {
	...
	if (ffb_int16_init(&pcm, aptx_pcm_samples * (mtu_write / aptx_code_len)) == -1 ||
			ffb_uint8_init(&bt, mtu_write) == -1) {
		error("Couldn't create data buffers: %s", strerror(ENOMEM));
		goto fail;
	}
	...
	for (;;) {
		 ...
		 * ring buffer, we will simply move unprocessed data to the front
		 * of our linear buffer. */
		ffb_rewind(&pcm, samples - input_len);
...
void *io_thread_a2dp_source_ldac(void *arg) {
	...
	if (ffb_int16_init(&pcm, ldac_pcm_samples) == -1 ||
			ffb_uint8_init(&bt, t->mtu_write) == -1) {
		error("Couldn't create data buffers: %s", strerror(ENOMEM));
		goto fail;
	}
	...
	for (;;) {
		...
		while (input_len >= ldac_pcm_samples) {
			...
			if (encoded && io_thread_write_bt(t->bt_fd, bt.data, encoded) == -1) {
				if (errno == ECONNRESET || errno == ENOTCONN) {
					/* exit thread upon BT socket disconnection */
					debug("BT socket disconnected: %d", t->bt_fd);
					goto fail;
...
void *io_thread_sco(void *arg) {
	...
	if (ffb_uint8_init(&bt_in, 128) == -1 || ffb_uint8_init(&bt_out, 128) == -1) {
